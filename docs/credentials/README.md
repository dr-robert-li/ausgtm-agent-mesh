...
| Webflow | `direct_api` | read (`webflow_list_cms_items`) + approval-gated draft create (`webflow_create_cms_item`) | `WEBFLOW_API_TOKEN` | [webflow.md](./webflow.md) | `tests/test_webflow_live.py` skips; adapter falls back to stub |
...
| Clockify | `direct_api` | read-only (`clockify_read_time_entries`) | `CLOCKIFY_API_KEY` | [clockify.md](./clockify.md) | `tests/test_clockify_live.py` skips; adapter falls back to stub |
