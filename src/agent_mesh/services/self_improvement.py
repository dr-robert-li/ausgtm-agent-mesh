...
def reflect_on_task(
    ...
    *,
    ...
    # Bind the patch hash to the proposal IDENTITY, not just the content (WR-07).
    # Two distinct proposals with identical proposed_patch text would otherwise
    # share a hash, letting one APPROVED record satisfy is_approved() for the other.
    # proposal_id is generated by the model constructor, so the hash is bound after
    # construction. promote_proposal hashes the SAME {proposal_id, proposed_patch}
    # shape for its is_approved() check.
    proposal = proposal.model_copy(
        update={
            "patch_hash": approvals.payload_hash(
                _patch_hash_payload(proposal.proposal_id, proposed_patch)
            )
        }
    )
...
def _patch_hash_payload(proposal_id: str, proposed_patch: str) -> dict:
    """Canonical payload bound by the promotion approval hash (WR-07).

    Binds the approval to a specific artifact identity (proposal_id) AND its
    content, so an approval cannot be replayed against a different proposal that
    merely happens to carry the same patch text."""
    return {"proposal_id": proposal_id, "proposed_patch": proposed_patch}
...
def promote_proposal(
    ...
    *,
    ...
    if not approvals.is_approved(
        record, _patch_hash_payload(proposal.proposal_id, proposal.proposed_patch)
    ):
        raise PromotionRefused(
