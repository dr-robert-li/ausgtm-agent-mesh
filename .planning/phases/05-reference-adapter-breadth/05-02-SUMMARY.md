...
- The full `make test` / `pytest -m "not live"` suite has **pre-existing** collection errors and failures in unrelated modules (opentelemetry/langgraph/litellm optional deps not installed in this worktree env): `test_observability_otel.py`, `test_tool_span.py`, `test_trace_propagation.py`, `test_checkpointer_resume.py`, `test_budget_halt_governed.py` (collection errors) and `test_model_gateway_router.py`, `test_orchestration_graph.py`, `test_read_path.py` (failures). Verified these are identical on the clean base commit (`19 failed, 170 passed` at base → `19 failed, 175 passed` with this plan's 5 new tests). They are out of scope (SCOPE BOUNDARY: only auto-fix issues directly caused by this task) — not introduced by, and not touching, any of the four Webflow-owned files. The Webflow default lane (`tests/test_webflow_adapter.py`) is fully green and the live lane collects + skips cleanly.
