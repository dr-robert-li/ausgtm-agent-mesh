...
The codebase is **fully synchronous** (TaskService, Worker, sessions, the `Repository` protocol). Therefore RepositorySQL must use **sync psycopg3 + `psycopg_pool.ConnectionPool`** — not async. The async psycopg gotchas in project memory (`AsyncConnectionPool`, `AsyncPostgresSaver.setup()` + `autocommit=True`) belong to the **LangGraph checkpointer in Phase 2 (ORCH-02)**, not to this phase's repository. SEC-01 mirrors `slack_verify.py`'s crypto (stdlib `hmac`/`hashlib`/`compare_digest`) with one deliberate divergence: the approval gate **fails closed** when no secret is set (unlike Slack's dev fail-open), because a missing secret here re-opens the exact Critical bypass this phase exists to close.

**Primary recommendation:** In 01-01, first widen the `Repository` protocol (tenant_id on `list_events`/`list_evaluations`; add `list_tool_calls`/`list_approvals`), migrate all private-state reach-ins in `runner.py`/tests/`smoke.py`, update `InMemoryRepository`, then implement `RepositorySQL` (sync psycopg3 `ConnectionPool`) selected by `DATABASE_URL` in `get_repository()`. 01-03 mirrors `slack_verify.py`'s crypto with an HMAC approval token binding `approval_record_id|payload_hash|requester_id|exp`, enforced (fail-closed) at **both** `/v1/approvals` and the MCP `submit_approval` tool, with the recorded `approver_id` derived from the verified token/channel rather than the request body. 01-02 wires/validates the already-existing Pub/Sub path against the local emulator. Build order: **01-01 → 01-03** (SEC-02 resume tests sit on the resume path 01-01 fixes); **01-02 is independent**.
...
- **The requester approves their own write** (CLAUDE.md §3: "deliver approval prompts to the respective requester"). The expected approver is known at issue time (`ApprovalRequest.requester_id`) — bind it into the token so SEC-01 rejects a *self-asserted* `approver_id`, not merely an unauthenticated request.
...
- Token TTL (recommend 1h; 60s–1h acceptable).
...
|----|-------------|------------------|
| DUR-01 | Postgres-backed repository replaces the in-memory singleton; tasks/sessions/approvals/tool-calls survive a worker restart | `RepositorySQL` (sync psycopg3 `ConnectionPool`) mapped column-by-column to `0001`/`0002` tables (see Pattern 2). Selected by `DATABASE_URL` in `get_repository()`. **Blocker dependency:** protocol must be widened + private reach-ins migrated first (Don't Hand-Roll #1). |
...
| SEC-01 | `/v1/approvals` verifies a signed approval token (HMAC/JWT) issued at request time; forged/self-asserted `approver_id` rejected | Mirror `slack_verify.py` crypto (stdlib HMAC), **fail-closed**. Token binds `approval_record_id|payload_hash|requester_id|exp`, issued in `open_approval()`, verified at endpoint; recorded `approver_id` derived from the verified token, NOT the body. Must cover MCP `submit_approval` too. See SEC-01 section. |
...
| stdlib `hmac` + `hashlib` + `secrets` | Python 3.11+ stdlib `[VERIFIED: codebase grep]` | HMAC-signed approval token (SEC-01) | Mirrors the *existing, tested* `slack_verify.py` crypto. Zero new dependency. |
        ...
        └──────────────────────────────────────┘             │
            ...
            worker/main.py  ──► Worker.process(task_id) ◄─────┘
              ...
              _run_inprocess (drain)   │
                                       ▼
                ToolCall + open_approval → issue token         SEC-01
                (token binds record_id|payload_hash|requester_id|exp)
                                       │ task → AWAITING_APPROVAL
                                       ▼
   ┌─────────── approval decision arrives ───────────┐
   │ POST /v1/approvals  (HTTP) ── verify token (fail-closed) │ SEC-01
   │ MCP submit_approval tool   ── verify token (fail-closed) │ SEC-01 (MUST also gate)
   │   approver_id := token.requester_id  (body value ignored)│
   └──────────────────────────────────────────────────────────┘
...
├── api/
│   ├── app.py               # /v1/approvals: verify token (fail-closed) before record_decision; approver_id from token
│   └── mcp_server.py        # submit_approval tool: add approval_token arg; same verify (DO NOT MISS)
...
└── test_approval_security.py # NEW: SEC-01/SEC-02; MUST set APPROVAL_SIGNING_SECRET (unlike rest of suite)
```
...
**What:** One module-level `ConnectionPool`; each method borrows a connection via `with self._pool.connection() as conn:` and uses `conn.execute(...)` / `cursor.fetchone()`. JSONB columns adapted with `psycopg.types.json.Jsonb`. **Multi-write methods (e.g. `transition_task` = UPDATE tasks + INSERT task_events) run inside a single transaction** — see Pitfall 7.
**Example:**
```python
...
class RepositorySQL:
    ...
    def transition_task(self, task_id, target, note=None):
        # Single transaction: UPDATE state AND append the audit event atomically (Pitfall 7).
        with self._pool.connection() as conn:   # psycopg3: `with conn` = one transaction
            row = conn.execute(
                "UPDATE tasks SET state=%s, updated_at=now() WHERE task_id=%s "
                "RETURNING tenant_id", (str(target), task_id)).fetchone()
            conn.execute(
                "INSERT INTO task_events (event_id, task_id, tenant_id, state, note) "
                "VALUES (%s,%s,%s,%s,%s)",
                (uuid4().hex, task_id, row[0], str(target), note))
...
### Pattern 3: Stateless HMAC approval token (SEC-01) — mirrors slack_verify.py crypto, fails CLOSED
**What:** When `open_approval()` persists a pending record, also mint a token binding `approval_record_id`, `payload_hash`, **`requester_id`** (the expected approver), and an expiry. The verifier at the endpoint recomputes the HMAC and re-loads the `ApprovalRecord` to confirm binding. The endpoint then records `approver_id` **from the verified token's `requester_id`**, ignoring any body-supplied value. No new storage.

**Two deliberate divergences from `slack_verify.py`:**
1. **Fail CLOSED on missing secret.** `slack_verify` returns `True` with no secret (dev convenience). The approval gate must return `False` — a missing `APPROVAL_SIGNING_SECRET` otherwise re-opens the exact Critical bypass this phase closes. Tests therefore MUST set the secret (see SEC-02 note).
2. **Bind + derive the approver.** SEC-01 says reject a *self-asserted* `approver_id`. Binding `requester_id` and deriving the recorded approver from the token (not the body) is what makes that true.

**Example:**
```python
# Source: crypto mirrors src/agent_mesh/api/slack_verify.py (stdlib hmac) [VERIFIED: codebase]
import hashlib, hmac, os, time

def _approval_secret(secret: str | None) -> str:
    return secret if secret is not None else os.getenv("APPROVAL_SIGNING_SECRET", "")

def issue_approval_token(record: ApprovalRecord, requester_id: str, *,
                         ttl_s: int = 3600, secret: str | None = None) -> str:
    secret = _approval_secret(secret)
    exp = int(time.time()) + ttl_s
    msg = f"{record.approval_record_id}:{record.payload_hash}:{requester_id}:{exp}".encode()
    sig = hmac.new(secret.encode(), msg, hashlib.sha256).hexdigest()
    return f"{record.approval_record_id}.{requester_id}.{exp}.{sig}"

def verify_approval_token(token: str, record: ApprovalRecord, *,
                          secret: str | None = None) -> str | None:
    """Return the verified approver_id (token's requester_id) on success, else None.
    FAILS CLOSED: no secret configured -> None (reject)."""
    secret = _approval_secret(secret)
    if not secret:
        return None                                   # FAIL CLOSED (diverges from slack_verify)
    try:
        rid, approver_id, exp_s, sig = token.split(".")
        exp = int(exp_s)
    ...
    if time.time() > exp:                             # expiry / replay window
        return None
    msg = f"{record.approval_record_id}:{record.payload_hash}:{approver_id}:{exp}".encode()
    expected = hmac.new(secret.encode(), msg, hashlib.sha256).hexdigest()
    if not hmac.compare_digest(expected, sig):        # constant-time, mirrors slack_verify
        return None
    return approver_id                                # the ONLY trusted approver identity
```
**Endpoint enforcement (`app.py` AND `mcp_server.py`):**
```python
# app.py /v1/approvals — verify BEFORE record_decision; approver_id comes from the TOKEN, not the body.
record = _service.repo.get_approval(payload["approval_record_id"])
if record is None:
    raise HTTPException(404, "unknown approval")
approver_id = verify_approval_token(payload.get("approval_token", ""), record)
if approver_id is None:
    raise HTTPException(401, "invalid or missing approval token")
# Body 'approver_id' is IGNORED — the verified token's requester is the recorded approver.
record = _service.submit_approval_decision(
    ...
    approver_id=approver_id,            # from token, not payload["approver_id"]
    channel=payload.get("channel", "api"),
)
```
**Issue-time wiring:** `open_approval(repo, request)` already has `request.requester_id` — return the token alongside the record (e.g. `open_approval` returns `(record, issue_approval_token(record, request.requester_id))`, or the worker calls `issue_approval_token` right after `open_approval` and attaches the token to the Slack/MCP/API approval prompt).
...
- **Recording the body-supplied `approver_id`.** That is the exact existing bug. The verified token's `requester_id` is the only trusted approver identity; the body value is ignored.
- **Fail-open on missing approval secret.** Copying `slack_verify`'s `return True` into the approval gate re-opens the Critical bypass. Fail CLOSED.
...
| Multi-statement atomicity | Separate auto-committed UPDATE + INSERT | `with conn:` transaction block (psycopg3) | `transition_task` must keep `tasks.state` and the `task_events` audit row consistent on crash (Pitfall 7). |
| Signed token format | Custom serialization / encryption | stdlib `hmac` + `compare_digest` (mirror `slack_verify.py` crypto) | Constant-time comparison and the exact pattern already proven & tested in this repo. |
...
| Secrets/env vars | NEW env var `APPROVAL_SIGNING_SECRET` (code reads it; mirrors `SLACK_SIGNING_SECRET`). `DATABASE_URL`, `USE_PUBSUB`, `TASK_SUBSCRIPTION`, `PUBSUB_EMULATOR_HOST` already referenced. Add `APPROVAL_SIGNING_SECRET` to `.env.example` and the deploy secret manifest. | Add new secret name to settings + `.env.example` + `scripts/gcp_deploy_core.sh --set-secrets` list. Because the gate fails closed, the worker/ingress MUST have this secret set in any non-trivial run. |
...
**How to avoid:** Add an `approval_token` parameter to the MCP `submit_approval` tool and run the same `verify_approval_token`; derive `approver_id` from the token. (For MCP the token's `requester_id` should be the `mcp:{mcp_subject}` issued at open time.)
...
### Pitfall 7: `transition_task` is two writes — keep them atomic
**What goes wrong:** `InMemoryRepository.transition_task` updates the task state AND appends a `TaskEvent` under one lock. A naive `RepositorySQL` that issues two separately-committed statements can crash between them, leaving `tasks.state` advanced but the `task_events` audit row missing (or vice-versa) — breaking success-criterion-1's "resumes with no lost state" audit trail.
**Why it happens:** psycopg3 auto-commits each `conn.execute` outside an explicit transaction block.
**How to avoid:** Wrap the UPDATE + INSERT in a single `with conn:` transaction (Pattern 2). Same applies to any method doing >1 write.
**Warning signs:** Restart-survival test shows a task in `approved` state with no corresponding `task_events` row.
...
- **Mechanism:** stdlib HMAC-SHA256, self-contained `record_id.requester_id.exp.sig` (Pattern 3). No PyJWT, no new storage.
- **Binding:** `approval_record_id` + `payload_hash` + **`requester_id`** + `exp`. `approval_record_id` binding makes the token single-task → no cross-task replay (SEC-02b); `requester_id` binding + token-derived `approver_id` rejects a self-asserted approver (SEC-01).
- **Fail-closed:** `verify_approval_token` returns `None` (reject) when no secret is configured — diverges deliberately from `slack_verify`'s dev fail-open. This is why `test_approval_security.py` must set `APPROVAL_SIGNING_SECRET`.
- **Issued:** in/after `approvals.open_approval()` using `request.requester_id`, when the approval request is opened — so the token travels with the approval prompt to Slack/MCP/API.
- **Recorded approver:** the endpoint passes the **token-derived** `requester_id` into `submit_approval_decision(approver_id=...)`; the request body's `approver_id` is ignored entirely.
- **Secret:** new `APPROVAL_SIGNING_SECRET` env var; add `approval_signing_secret_env: str = "APPROVAL_SIGNING_SECRET"` to `Settings` mirroring `slack_signing_secret_env`.
- **Channel delivery:**
  ...
  - **API:** token returned in the create-approval/open response; caller echoes it in the `/v1/approvals` body as `approval_token`.
...
| (c) forged / unauthenticated POST rejected | POST to `/v1/approvals` (and MCP `submit_approval`) with no/invalid token, or a self-asserted `approver_id`, → 401, no ledger mutation | **New** `verify_approval_token` (fail-closed) at both ingress points; `approver_id` from token | New endpoint guard. |

**Test file `tests/test_approval_security.py` (extends `test_approval_gating.py` patterns, uses `repo` fixture + FastAPI `TestClient`; MUST set `APPROVAL_SIGNING_SECRET` via env/monkeypatch — unlike the rest of the suite, because the gate fails closed):**
1. `test_valid_token_accepted` — issue token, POST, decision recorded with the token's requester as approver.
2. `test_forged_approver_id_rejected` — POST with a spoofed body `approver_id` and no/invalid token → 401; even with a valid token, the recorded approver is the token's requester, not the body value (the exact CONCERNS.md Critical #1).
3. `test_missing_secret_rejects` — unset `APPROVAL_SIGNING_SECRET` → 401 (proves fail-closed; guards against the Slack-style fail-open regression).
4. `test_tampered_token_rejected` — flip a sig char → 401 (mirrors `test_tampered_body_rejected`).
5. `test_expired_token_rejected` — `ttl_s` in the past → 401.
6. `test_mutated_payload_invalidates_approval` — approve, then mutate `ToolCall.parameters`, resume → tool call NOT executed (SEC-02a; reuse worker resume path).
7. `test_approval_not_replayable_across_tasks` — token for task A's record → reject when presented for task B's record_id (SEC-02b).
8. MCP parity: `test_mcp_submit_approval_requires_token` — same forged/valid checks against the MCP tool path.

**Existing coverage to build on:** `test_approval_gating.py` already proves payload-hash binding (`test_payload_hash_binds_to_exact_payload`) and the pause→approve→complete flow; `test_slack_verify.py` is the template for the token tests (valid/tampered/stale). CONCERNS.md "Test Coverage Gaps" explicitly lists the unauthenticated-POST, replay, and mutation cases as currently missing — this file closes them.
...
| Hand-rolled token crypto | stdlib `hmac.compare_digest` (constant-time) | stable since 3.3 | Mirror `slack_verify.py` crypto; no third-party crypto. |
...
| A5 | The expected approver = `ApprovalRequest.requester_id` (requester approves own write, per CLAUDE.md §3) | SEC-01 | Med — if a deployment routes approvals to a *different* approver than the requester, the token binding must use that approver id instead. Flag to planner; for the POC the requester-approves model holds. |
...
**Missing dependencies with fallback:** local Postgres and the Pub/Sub emulator are validation-only; unit tests run fully on the in-memory/in-process fakes (skip pg/emulator tests when their env vars are unset — matches TESTING.md "no external deps for tests"). Note the SEC tests are the exception: they need `APPROVAL_SIGNING_SECRET` set (no external service, just an env var).
...
|---------------|---------|-----------------|
| V2 Authentication | yes | HMAC-signed approval token authenticates the approver action; recorded `approver_id` derived from the token, never the body (replaces self-asserted `approver_id`). |
| V3 Session Management | partial | Token expiry (`exp`) bounds the approval window (recommend 60s–1h TTL). |
| V4 Access Control | yes | Token bound to `approval_record_id` + `requester_id` → an approval authorizes exactly one task's write by exactly one approver (no cross-task reuse, no approver spoofing). |
...
| V6 Cryptography | yes | stdlib `hmac` + `compare_digest` (constant-time). Never hand-roll comparison or use `==` on signatures. Gate fails closed on missing key. |
...
|---------|--------|---------------------|
| Forged approval (self-asserted `approver_id`) | Spoofing / Elevation of Privilege | Token binds `requester_id`; recorded approver derived from verified token, body ignored (SEC-01) — at HTTP and MCP. |
...
| Missing-secret fail-open | Spoofing | `verify_approval_token` fails closed (returns None) when no secret set. |
...
| Audit-trail divergence on crash | Repudiation | `transition_task` UPDATE+INSERT in one transaction (Pitfall 7). |
...
01-01 (DUR-01, DUR-02)  ── LINCHPIN ──┐
   ...
   4. Implement RepositorySQL (sync psycopg3 ConnectionPool) over 0001/0002
      — transition_task UPDATE+INSERT in ONE transaction (Pitfall 7)
...
01-03 (SEC-01, SEC-02)  ── depends on 01-01 ──
   ...
   issue/verify token (fail-closed, binds requester_id) in approvals.py;
   enforce at app.py AND mcp_server.py; approver_id from token, not body;
   test_approval_security.py (8 cases above; sets APPROVAL_SIGNING_SECRET).
...
- SEC token design: HIGH — crypto mirrors tested `slack_verify.py`; approver-binding + fail-closed + token-derived approver close SEC-01's exact wording; concrete code given.
