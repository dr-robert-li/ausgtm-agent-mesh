...
must_haves:
  truths:
    ...
    - "Model, tool, AND approval events emit OTel spans carrying the shared request metadata (trace_metadata() dict) as span attributes — correlated under one trace (OBS-01)"
    - "OrchestrationResult.trace_id is SET per task (closes the P2 gap); the dataclass shape stays stable for runner/ledger/tests"
    - "W3C traceparent is stored on the task record at ingress and restored in the worker so approval spans (ingress) and model spans (worker) land in ONE trace across the Pub/Sub boundary (OTel context does NOT cross Pub/Sub)"
  ...
  artifacts:
    ...
    - path: "src/agent_mesh/worker/orchestrator.py"
      provides: "OrchestrationResult.trace_id SET in _run_langgraph and resume_mesh (was unset in P2)"
      contains: "trace_id"
    ...
    - path: "tests/test_trace_propagation.py"
      provides: "OBS-01: traceparent stored on task record (ingress) restored in worker → one trace"
  ...
  key_links:
    - from: "src/agent_mesh/worker/orchestrator.py"
      ...
      via: "set from the per-task OTel root (langfuse.create_trace_id / get_current_trace_id)"
      pattern: "trace_id ="
...
THE CROSS-PROCESS SEAM (RESEARCH Pitfall 4): approval spans fire on the ingress/API side, model/tool
spans fire in the worker, with Pub/Sub between them (DUR-03). OTel ambient context does NOT cross that
boundary — store the W3C `traceparent` on the task record at ingress, restore it in the worker before the
root span, or approval and model events land in SEPARATE traces and OBS-01 "correlate" fails.

Purpose: OBS-01 + OBS-02 — every model/tool/approval event correlates under shared metadata; prompt/eval configured.
Output: v4 migration + OTel init + trace_id wiring + traceparent propagation + prompt fetch-with-fallback + live seed.
</objective>
...
<task type="auto" tdd="true">
  <name>Task 2: Span emission + trace_id + cross-process traceparent (OBS-01 correlation)</name>
  <read_first>
  ...
  <behavior>
    ...
    - An approval recorded on the ingress side emits a span that lands under the SAME trace as the worker's model spans, via traceparent stored on the task record and restored in the worker
  ...
  <action>
    In `graph.py`, attach the v4 CallbackHandler to the graph run via `config={"callbacks":[handler]}` (handler
    from `observability.get_langchain_callback`, None-safe), so node/model spans nest under the task trace. In
    `orchestrator.py`, SET `OrchestrationResult.trace_id` (currently unset, lines 242/305) from the per-task OTel
    root (`langfuse.create_trace_id()` / `get_current_trace_id()`); keep the dataclass shape stable. Open the
    root span in the worker and set the `trace_metadata()` dict as span attributes. Implement cross-process
    propagation: at ingress store the W3C `traceparent` on the task record (use an existing task field or add a
    `traceparent` field consistent with the contracts), restore it in the worker before opening the root span
    (RESEARCH Pitfall 4 — OTel context does NOT cross Pub/Sub). In `approvals.py` wrap `open_approval`/
    `record_decision` with a READ-ONLY OTel span carrying `trace_metadata()` attrs — the span is telemetry ONLY:
    do NOT branch on span data, do NOT alter `verify_approval_token`/`payload_hash`/`is_approved`/replay guard;
    the approver is derived solely from the signed token (RF-1 / SEC-01/02).
  </action>
  <verify>
  ...
  <acceptance_criteria>
    ...
    - `tests/test_trace_propagation.py` proves an ingress approval span and worker model spans share one trace via stored→restored traceparent
  ...
  <done>Model/tool/approval events correlate under one trace_id with shared-metadata attrs; SEC-01/02 intact.</done>
</task>
...
|----------|-------------|
| ingress (approval span) → worker (model spans) | crosses Pub/Sub; correlation via stored traceparent |
...
per-task trace_id with shared metadata (in-memory exporter, no server), traceparent crosses Pub/Sub, the
dead langfuse v2 import is fixed, and prompt/version + dataset + example eval are seeded — default suite green, live lane opt-in.
</success_criteria>
